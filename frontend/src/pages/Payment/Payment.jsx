//Payment.jsx
import { useLocation } from 'react-router-dom';
import PayPalCheckoutButton from '../../components/Payment/PayPalButton';

  const Payment = () => {
    const location = useLocation();
    const { orderId } = location.state || {};
  console.log("Order ID:", orderId);
  console.log("Location state in Payment:", location.state);

  return (
    <main className="main flex flex-col gap-6 bg-[#FCE3D7] text-center md:gap-10 lg:flex-row lg:gap-0">
      <section className="sell-section flex flex-col gap-6 pt-12 text-center lg:w-1/3 lg:pb-4 lg:pt-0">
        <div className="flex w-full flex-col gap-4 sm:gap-8 lg:h-full lg:items-center lg:justify-center">
          <h1 className="h2">Réglez votre commande</h1>
        </div>
      </section>

   {/* Bouton de paiement PayPal */}
      <section className="params-product-form flex items-center justify-center gap-4 pb-8 sm:gap-6 lg:ml-[33%] lg:flex lg:w-3/4 lg:flex-col lg:bg-white lg:pb-8 lg:pt-16">
        <div className="flex flex-col items-center w-full">
          <h1 className="h2">Choisissez votre mode de règlement.</h1>
          <div className="mt-28 w-full px-4 flex justify-center">
              <PayPalCheckoutButton orderId={orderId} className=" mt-200 w-full max-w-xs" />
          </div>
        </div>
      </section>
    </main>
  );
}

export default Payment;






